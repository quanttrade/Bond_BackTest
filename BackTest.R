...
BondQuote_Cut <- BondQuote_Orig %>% filter(InnerCode %in% distinct(Signal_Total,InnerCode)$InnerCode) %>%
...
for(cursor in 1:nrow(RebalanceDate_Backtest)){ 
  ...
  if(cursor ==1 ){#第一期单独处理
    Real_Position[[cursor]] <- Signal_Total %>% filter(Date==TradeDate_) %>% 
      left_join(BondQuote_Cut, by = c('InnerCode','Date'='TradingDay')) %>% #关联上当日行情计算购买量
    ...
    if(sum(Position_Matrix$Weight_LT!=Position_Matrix$Weight)==0){
      ...
      Real_Position[[cursor]] <- Signal_Total %>% filter(Date==TradeDate_) %>% 
        ...
        left_join(BondQuote_Cut, by = c('InnerCode','Date'='TradingDay')) %>% #关联上当日行情，对于PR债，前收盘价已经调整
      ...
      Prev_Position_Today <- Prev_Position_[c('InnerCode','Weight','Asset','Shares')] %>% 
        ...
        left_join(BondQuote_Cut, by = c('InnerCode','Date'='TradingDay')) %>% 
      ...
      Target_Position_ <- Target_Position_ %>% 
        left_join(BondQuote_Cut, by = c('InnerCode','Date'='TradingDay')) %>% #关联上当日行情，计算购买量 
        #Asset为根据Weight分配的资金，Shares为根据收盘价买入后持有的股数
